...
description = "Fadal post with probing and accublast R43853";
vendor = "Fadal";
vendorUrl = "";
legal = "";
...
longDescription = "Milling post with Renishaw probing for Fadal.";
...
properties = {
    ...
    hasAccublast: {
      title      : "Has Accublast Coolant Nozzle",
      description: "Select 'Yes' to enable pulse signal to change tool number on Accublast",
      group      : "configuration",
      type       : "boolean",
      value      : false,
      scope      : "post"
...
// Probing cycle inputs
var probeR0 = createFormat({prefix:"R0", decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // Overtravel
var probeR1 = createFormat({prefix:"R1", decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // X feature size or X surface position
var probeR2 = createFormat({prefix:"R2", decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // Y feature size or Y surface position
var probeR3 = createFormat({prefix:"R3", decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // Z surface position or depth to probe at
var probeR4 = createFormat({prefix:"R4", decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // Diameter of Bore or Boss
var probeR5 = createFormat({prefix:"R5", decimals:0, forceDecimal:true, forceSign:true}); // Radius Offset to store size of feature. Can only be R5+80(D80) to R5+84(D84)
var probeR6 = createFormat({prefix:"R6", decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // Clearance for Boss/Web cycles only. It equals the standoff amount of clearance before gauging of the surface takes place. Default 5mm (0.2in)
var probeR7 = createFormat({prefix:"R7", decimals:0, forceDecimal:true, forceSign:true}); // Work Offset number to be updated. Can only be R7+1(E1) to R7+6(E6)
var probeR8 = createFormat({prefix:"R8", decimals:0, forceDecimal:true, forceSign:true}); // Length (H) or radius(R) offset to be updated. Can only be R8+1(H1 or D1) to R8+30(H30 or D30)
var probeR9 = createFormat({prefix:"R9", decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // Equals half the tolerance of the feature being measured
var probeFormat = createFormat({decimals:(unit == MM ? 3 : 4), forceDecimal:true, forceSign:true}); // Equals half the tolerance of the feature being measured
...
/**
  Returns the parameters for using Fadal Renishaw toolsetting macros.
  1 for length only
  2 for 
*/
function getFadalToolType(toolType) {
  switch (toolType) {
  case TOOL_DRILL:
  case TOOL_REAMER:
  ...
  case TOOL_MILLING_FORM:
    return 1; // only length, no shifting
  case TOOL_MILLING_FACE:
    return 2; // length but shift and rotating
  ...
  case TOOL_MILLING_LOLLIPOP:
    return 4; // ball nose
  default:
    error(localize("Trying to measure invalid Fadal tool type."));
    return -1;
  }
}

function getToolSetterArguments(tool, isBrokenCheck) {
	
  switch (getFadalToolType(tool.type)) {
  
  case 1:
    return [
		probeR0.format(1),
        probeR1.format(tool.number),
		(isBrokenCheck ? probeR4.format(getProperty("toolBreakageTolerance")) : probeR2.format(tool.bodyLength + tool.holderLength)),
  ...
  case 2:
	return [
        ...
        probeR1.format(tool.number),
		(isBrokenCheck ? probeR4.format(getProperty("toolBreakageTolerance")) : probeR2.format(tool.bodyLength + tool.holderLength)),
		(!tool.clockwise ? probeR3.format(-1 * tool.diameter) : probeR3.format(tool.diameter))
	];
  case 3:
	return [
		(isBrokenCheck ? probeR0.format(1) : probeR0.format(3)),
        probeR1.format(tool.number),
		(isBrokenCheck ? probeR4.format(getProperty("toolBreakageTolerance")) : probeR2.format(tool.bodyLength + tool.holderLength)),
		(!tool.clockwise ? probeR3.format(-1 * tool.diameter) : probeR3.format(tool.diameter))
  ...
  default:
    error(localize("Invalid Fadal tool type."));
    return -1;
  }
}
...
function writeToolMeasureBlock(tool) {
 // var writeFunction = measureTool ? writeBlock : writeOptionalBlock;
  var comment = measureTool ? formatComment("MEASURE TOOL") : "";
      writeBlock(mFormat.format(98),"P" + 9952); // Clear Variables
	  writeBlock(
        mFormat.format(98),
        "P" + 9953,
		getToolSetterArguments(tool, false),
		comment
      ); // probe tool
...
function onSection() {
  ...
  // custom block skip
	writeln("");
	//writeOptionalBlock("#GOTO:N" + currentSection.getId());
	writeln("/ #GOTO:N" + currentSection.getId()); // had to remove line number because it would always read
	writeBlock("#GOTO:N" + currentSection.getId()+"END");
    writeBlock("#:N" + currentSection.getId());
  }
  ...
  if (insertToolCall || retracted) {
   ...
   if (isProbeOperation()) {
    ...
    writeBlock(mFormat.format(98), "P" + 9020); // spin the probe on
  }
  ...
  if (isFirstCyclePoint()) {
    ...
    case "boring":
	  ...
	  case "probing-x":
			...
			prepareProbing(cycle, probeWorkOffsetCode);
			writeBlock(
				probeR1.format(x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)),
				probeR0.format(cycle.probeOvertravel),
				probeR7.format(probeWorkOffsetCode),
				getProbingArguments(cycle, false)
			);
			writeBlock(mFormat.format(98), "P" + 9013); // Execute x
			updateProbingData(probeWorkOffsetCode);
      break;
    ...
    case "probing-y":
		...
		prepareProbing(cycle, probeWorkOffsetCode);
		writeBlock(
			probeR2.format(y + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)),
			probeR0.format(cycle.probeOvertravel),
			probeR7.format(probeWorkOffsetCode),
			getProbingArguments(cycle, false)
		 );
		writeBlock(mFormat.format(98), "P" + 9013); // Execute Y
		updateProbingData(probeWorkOffsetCode);
    ...
    case "probing-z":
		...
		prepareProbing(cycle, probeWorkOffsetCode);
		writeBlock(
			probeR3.format(z - cycle.depth),
			probeR0.format(cycle.probeOvertravel),
			probeR7.format(probeWorkOffsetCode),
			getProbingArguments(cycle, false)
		 );
		writeBlock(mFormat.format(98), "P" + 9018); // Execute Z
        updateProbingData(probeWorkOffsetCode);
      break;
    case "probing-x-wall":
      protectedProbeMove(cycle, x, y, z);
	  prepareProbing(cycle, probeWorkOffsetCode);
		writeBlock(
			probeR1.format(cycle.width1),
			probeR3.format(z - cycle.depth),
			probeR0.format(cycle.probeOvertravel),
			probeR6.format(cycle.probeClearance),
			probeR7.format(probeWorkOffsetCode),
			getProbingArguments(cycle, false)
		 );
     
		writeBlock(mFormat.format(98), "P" + 9010); // Execute x
		updateProbingData(probeWorkOffsetCode);
      break;
    ...
    case "probing-y-wall":
      protectedProbeMove(cycle, x, y, z);
		...
		writeBlock(mFormat.format(98), "P" + 9010); // Execute Y
		updateProbingData(probeWorkOffsetCode);
      break;
	  
    case "probing-x-channel":
		protectedProbeMove(cycle, x, y, z - cycle.depth); // Move into channel
		prepareProbing(cycle, probeWorkOffsetCode);
		writeBlock(
			probeR1.format(cycle.width1),
			probeR0.format(cycle.probeOvertravel),
    ...
    case "probing-x-channel-with-island":
	   error(localize("probing-x-channel-with-island is not supported with this cycle"));
      break;
    ...
    case "probing-y-channel-with-island":
       error(localize("probing-y-channel-with-island is not supported with this cycle"));
      break;
    ...
    case "probing-xy-circular-boss":
			...
			prepareProbing(cycle, probeWorkOffsetCode);	
			writeBlock(
				mFormat.format(98), "P" + 9019,
				probeR4.format(cycle.width1),
				probeR3.format(z - cycle.depth),
				probeR6.format(cycle.probeClearance),
				probeR0.format(cycle.probeOvertravel),
				probeR7.format(probeWorkOffsetCode),
    ...
    case "probing-xy-circular-partial-boss":
		error(localize("probing-xy-circular-partial-boss is not supported with this cycle"));
    ...
    case "probing-xy-circular-partial-hole":
		error(localize("probing-xy-circular-partial-hole is not supported with this cycle"));
      break;
    case "probing-xy-circular-hole-with-island":
		error(localize("probing-xy-circular-hole-with-island is not supported with this cycle"));
      break;
    case "probing-xy-circular-partial-hole-with-island":
		error(localize("probing-xy-circular-partial-hole-with-island is not supported with this cycle"));
      break;
    ...
    case "probing-xy-rectangular-hole-with-island":
	  error(localize("probing-xy-rectangular-hole-with-island is not supported with this cycle"));
      break;
    ...
    case "probing-xy-inner-corner":
			...
			prepareProbing(cycle, probeWorkOffsetCode);
			writeBlock(
				probeR2.format(y + approach(cycle.approach2) * (cycle.probeClearance + tool.diameter / 2)),
				probeR0.format(cycle.probeOvertravel),
				probeR7.format(probeWorkOffsetCode),
				getProbingArguments(cycle, false)
			);
    ...
    case "probing-xy-outer-corner":
		...
		var cornerNumber = 0;
		if (approach(cycle.approach1) == 1 && approach(cycle.approach2) == -1) cornerNumber = 1;
		if (approach(cycle.approach1) == -1 && approach(cycle.approach2) == -1) cornerNumber = 2;
		if (approach(cycle.approach1) == -1 && approach(cycle.approach2) == 1) cornerNumber = 3;
		if (approach(cycle.approach1) == 1 && approach(cycle.approach2) == 1) cornerNumber = 4;

		protectedProbeMove(cycle, x, y, z);
			prepareProbing(cycle, probeWorkOffsetCode);
			writeBlock(
				probeR1.format(x + approach(cycle.approach1) * (cycle.probeClearance * 2 + tool.diameter / 2)),
				probeR2.format(y + approach(cycle.approach2) * (cycle.probeClearance * 2 + tool.diameter / 2)),
				probeR3.format(z - cycle.depth),
				probeR4.format((y + approach(cycle.approach2) * (cycle.probeClearance + tool.diameter / 2)) + ((cycle.probeClearance / 2) * approach(cycle.approach2))),
				probeR8.format(cornerNumber),
				//probeR0.format(cycle.probeOvertravel),
				probeR7.format(probeWorkOffsetCode),
				getProbingArguments(cycle, false)
			);
			writeBlock(mFormat.format(98), "P" + 9008); // Execute x
    ...
    case "probing-x-plane-angle":
     error(localize("probing-x-plane-angle is not supported with this cycle")); 
      break;
	  
    case "probing-y-plane-angle":
       error(localize("probing-y-plane-angle is not supported with this cycle")); 
      break;
	  
    case "probing-xy-pcd-hole":
     error(localize("probing-xy-pcd-hole is not supported with this cycle")); 
      break;
	  
    case "probing-xy-pcd-boss":
      error(localize("probing-xy-pcd-boss is not supported with this cycle")); 
      break;
...
function prepareProbing(cycle) {
			var probeWCS = hasParameter("operation-strategy") && (getParameter("operation-strategy") == "probe");
			
			clearProbeVariables();
			writeBlock("#V50=" + currentWorkOffset);
			if (cycle.printResults) writeBlock("#V51=1"); // Output to serial
			writeBlock("#WAIT");
...
function getProbingArguments(cycle, probeWorkOffsetCode) {
		var probeWCS = hasParameter("operation-strategy") && (getParameter("operation-strategy") == "probe");
			return [
				...
				(cycle.outOfPositionAction == "stop-message" ? probeR9.format(cycle.tolerancePosition ? cycle.tolerancePosition : 0) : undefined),
...
function setProbingWorkset(x, y, probeOutputWorkOffset){
	
		
}

function updateProbingData(cycle, probeOutputWorkOffset) {
	if(probeOutputWorkOffset == currentWorkOffset) writeBlock(eFormat.format(probeOutputWorkOffset)); // Call offset again if current offset updated
	
}

function clearProbeVariables(){
		writeBlock(mFormat.format(98), "P" + 9009); // clear variables
}

  
function protectedProbeMove(_cycle, x, y, z) {
  var _x = probeR1.format(x);
  var _y = probeR2.format(y);
  var _z = probeR3.format(z);
  
  if (_z && z >= getCurrentPosition().z) {
		clearProbeVariables();
	    writeBlock(_z, probeR4.format(cycle.feedrate));
		writeBlock(mFormat.format(98), "P" + 9810); 
  }
  if (_x || _y) {
		clearProbeVariables();
		writeBlock(_x, _y, probeR4.format(highFeedrate));
		writeBlock(mFormat.format(98), "P" + 9810); 
  }
  if (_z && z < getCurrentPosition().z) {
		clearProbeVariables();
...
function onCycleEnd() {
	  if (isProbeOperation()) {
			...
			gMotionModal.reset();
      clearProbeVariables();
			writeBlock(mFormat.format(98), "P" + 9810, probeR3.format(cycle.retract), probeR4.format(cycle.feedrate)); // protected retract move
		} else {
			if (!cycleExpanded) {
...
function onCommand(command) {
  ...
  case COMMAND_BREAK_CONTROL:
	if (!toolChecked) { // avoid duplicate COMMAND_BREAK_CONTROL
      ...
      writeComment("CHECK FOR BROKEN TOOL:" + tool.number + " TYPE:" + tool.type + " CLOCKWISE:" + tool.isClockwise());
	 
	  writeBlock(mFormat.format(98),"P" + 9952); // Clear Variables
	  writeBlock(
        mFormat.format(98),
        "P" + 9953,
		getToolSetterArguments(tool, true)
      );
...
function onSectionEnd() {
  ...
  if (isProbeOperation(currentSection)) {
    writeBlock(mFormat.format(98), "P" + 9030); // spin the probe off
  ...
  // custom block skip
  if (currentSection.isOptional()) {
	writeBlock("#:N"+currentSection.getId()+"END");
  }
}
